...
namespace DeepOreIdentifier
{
	...
	{
		private static readonly float LabelOffsetFromMouse = 2f;

		private static readonly Color MessageBackgroundColour = new Color(0.15f, 0.15f, 0.15f, 0.8f);
		...
		private void DrawOreLabelOnMouseover()
		{
			...
			if (ore != null)
			{
				...
				Rect labelRect = new Rect(mousePosition.x - LabelOffsetFromMouse - labelSize.x, mousePosition.y - LabelOffsetFromMouse - labelSize.y, labelSize.x, labelSize.y);
				...
				if (Settings.TextBackground)
				{
					...
					GUI.color = MessageBackgroundColour;
